...
def _record_event(has_a, has_b):
    # Coincidence detection: 1 if BOTH cameras captured the same pulse, else 0
    return 1 if (has_a and has_b) else 0
...
def calculate_n(data, threshold, total_pairs=20):
    ...
    for a_val, b_val in zip(active_a, active_b):
        ...
        if in_event:
            # FIX HERE: Capture the returned count value
    ...
    if in_event:
        # FIX HERE: Capture the returned count value for a trailing event
        counts += _record_event(current_event_a, current_event_b)
...
def main():
    ...
    try:
        ...
        e1_err = np.sqrt((1 - e1**2) / sum(get_chsh_value(results, ang) for ang in angle_lists[0]))
        e2_err = np.sqrt((1 - e2**2) / sum(get_chsh_value(results, ang) for ang in angle_lists[1]))
        e3_err = np.sqrt((1 - e3**2) / sum(get_chsh_value(results, ang) for ang in angle_lists[2]))
        e4_err = np.sqrt((1 - e4**2) / sum(get_chsh_value(results, ang) for ang in angle_lists[3]))
        print(f"Individual Expectation Values:")
        print(f"  E1: {e1: .4f} | E2: {e2: .4f} | E3: {e3: .4f} | E4: {e4: .4f}\n with errors: E1_err: {e1_err:.4f} | E2_err: {e2_err:.4f} | E3_err: {e3_err:.4f} | E4_err: {e4_err:.4f}")
        ...
        s_errors = {
            formula: np.sqrt(e1_err**2 + e2_err**2 + e3_err**2 + e4_err**2)
            for formula in s_variants
        }

        print("Testing CHSH Configurations:") ## Brute forcing:
        for formula, s_val in s_variants.items():
            status = "VIOLATED!" if abs(s_val) > 2 else "Not Violated"
            print(f"  {formula:<24} => |S| = {abs(s_val):.4f} ({status}) \pm {s_errors[formula]:.4f}")
