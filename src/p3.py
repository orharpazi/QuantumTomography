...
VIDEO_DIR = ROOT_DIR / "data" / "raw" / "p3"
...
def process_video(
    ...
    try:
        ...
        if roi1 is not None and roi2 is not None:
            print("Skipping frame preview because both ROIs were pre-provided.")
        ...
        while True:
            ...
            if cam1_crop.size == 0 or cam2_crop.size == 0:
            ...
            avg1 = np.mean(gray1)
            avg2 = np.mean(gray2)
            data_log.append([timestamp, avg1, avg2])
        ...
        with output_filename.open(mode="w", newline="") as file:
            ...
            writer.writerow(["Timestamp", "Webcam_1_Average", "Webcam_2_Average"])
...
def main():
    for video_path in VIDEO_PATHS:
        process_video(video_path, output_filename=ROOT_DIR / "data" / "processed" / f"{video_path.stem}_intensities.csv")
